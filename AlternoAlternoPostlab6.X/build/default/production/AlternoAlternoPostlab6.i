# 1 "AlternoAlternoPostlab6.s"
# 1 "<built-in>" 1
# 1 "AlternoAlternoPostlab6.s" 2
...
# 13 "AlternoAlternoPostlab6.s" 2
 ...
 restart_tmr0 macro ; Reseteo del tmr0
    ...
    bcf ((INTCON) and 07Fh), 2 ; Bandera de overflow de tmr0, del INTCON BIT 2
    endm
 ...
 restart_tmr1 macro ; Reseteo del tmr1
    ...
    bcf ((PIR1) and 07Fh), 0 ; Bandera de overflow de tmr1
    endm

  restart_tmr2 macro; Reseteo del tmr1
    bcf ((PIR1) and 07Fh), 1 ; Bandera de overflow de tmr2
    endm
 ...
 isr:
    btfsc ((INTCON) and 07Fh), 2 ; Bandera de tmr0
    call int_t0 ; Interrupcion del tmr0

    btfsc ((PIR1) and 07Fh), 0 ; Bandera de tmr1
    call int_t1 ; Interrupcion del tmr1

    btfsc ((PIR1) and 07Fh), 1 ; Bandera de tmr2
 ...
 config_io:
    ...
    bcf TRISD,0 ; Port ((PORTD) and 07Fh), 0 - salida
    bcf TRISD,1 ; Port ((PORTD) and 07Fh), 1 - salida
...
config_tmr0:
    ...
    bcf ((OPTION_REG) and 07Fh), 0 ; PS = 000 = 1:2
...
config_tmr1:
    ...
    bsf ((T1CON) and 07Fh), 4 ; Prescale2 de 1:8
    bcf ((T1CON) and 07Fh), 3 ; low power off
    bcf ((T1CON) and 07Fh), 1 ; Reloj externo apagado
    bsf ((T1CON) and 07Fh), 0 ; Activa TMR!
...
config_tmr2:
    banksel PORTA
    bsf ((T2CON) and 07Fh), 6 ; 1
    bsf ((T2CON) and 07Fh), 5 ; 1
    bsf ((T2CON) and 07Fh), 4 ; 1
    bsf ((T2CON) and 07Fh), 3 ; 1 Postscaler 1:16

    bsf ((T2CON) and 07Fh), 2 ; Timer 2 On

    bsf ((T2CON) and 07Fh), 1 ;1
    bsf ((T2CON) and 07Fh), 0 ;1 prescaler 1:16

    banksel TRISB
    movlw 240 ; Precarga para el PR2
    movwf PR2 ; Ciclo de 500 ms
    clrf TMR2
    clrf ((PIR1) and 07Fh), 1 ; Limpiar bandera
    return


config_int_enable: ; Configurar interrupción del tmr0
    banksel TRISA
    bsf ((PIE1) and 07Fh), 0 ; Interrupcion tmr1
    bsf ((PIE1) and 07Fh), 1 ; Interrupcion tmr2

    banksel PORTA
    bsf ((INTCON) and 07Fh), 5 ; Interrupcion tmr0
    bcf ((INTCON) and 07Fh), 2 ; Bandera tmro
    bcf ((PIR1) and 07Fh), 0 ; Bandera tmr1
    bcf ((PIR1) and 07Fh), 1 ; Bandera tmr2
    bsf ((INTCON) and 07Fh), 6 ; Interrupciones perifericas
